...
def _cmd_adopt(args) -> int:
    ...
    if adopt_all and not bool(getattr(args, "yes", False)):
        print(
            f"curator: adopt {len(names)} unmanaged skill(s) into curator management?"
        )
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(
        f"curator: snapshot created at {display_hermes_home()}/skills/.curator_backups/{snap.name}"
    )
    return 0
...
def _cmd_purge(args) -> int:
    ...
    if ttl_days is None:
        ttl_days = int(
            cfg_get(load_config(), "curator", "archive_ttl_days", default=0) or 0
        )
    if ttl_days <= 0:
        print(
    ...
    if not getattr(args, "yes", False):
        try:
            ans = (
                input(f"Permanently delete {len(candidates)} archived skill(s)? [y/N] ")
                .strip()
                .lower()
            )
        except (EOFError, KeyboardInterrupt):
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        touched = {
            i.get("path")
            for i in (entry.get("before") or []) + (entry.get("after") or [])
        }
        print(f"  files:  {len(touched)}")
        if not getattr(args, "yes", False):
            try:
                ans = (
                    input("Restore this mutation's before-state? [y/N] ")
                    .strip()
                    .lower()
                )
            except (EOFError, KeyboardInterrupt):
...
def _cmd_usage(args) -> int:
    ...
    for r in rows:
        counts[r.get("provenance", "agent")] = (
            counts.get(r.get("provenance", "agent"), 0) + 1
        )
    print(
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of " + display_hermes_home() + "/skills/ "
    ...
    p_rollback = subs.add_parser(
        ...
        help="Restore "
        + display_hermes_home()
        + "/skills/ from a curator snapshot, or a single "
