...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _redispatch_held_inbound(
        ...
        if (
            prior is not None
            and prior is not asyncio.current_task()
            and not prior.done()
        ):
            prior.cancel()
            try:
    ...
    def _is_private_dm_topic_send(
        ...
        if metadata and metadata.get("telegram_dm_topic_created_for_send"):
            return False
        return bool(
            thread_id and metadata and metadata.get("telegram_dm_topic_reply_fallback")
        )
        ...
        try:
            ...
            if self._looks_like_network_error(err):
                ...
                logger.warning(
                    ...
                    self.name,
                    _redact_telegram_error_text(err),
                )
                self._send_path_degraded = True
                return False
            raise
        ...
        *,
        ...
        try:
            ...
            if self._looks_like_polling_conflict(err):
                ...
                loop = asyncio.get_running_loop()
                self._polling_error_task = loop.create_task(
                    self._handle_polling_conflict(err)
                )
                self._background_tasks.add(self._polling_error_task)
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        try:
            if not app:
                raise RuntimeError(
                    "Telegram application was torn down during reconnect"
                )
            ...
            if not self.has_fatal_error and not getattr(
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        if self._polling_conflict_count <= MAX_CONFLICT_RETRIES:
            ...
            try:
                ...
                if self._polling_conflict_count < MAX_CONFLICT_RETRIES and not getattr(
                    self, "_polling_teardown_started", False
                ):
    ...
    async def _on_platform_update(self, update, context) -> None:
        ...
        try:
            ...
            logger.debug(
                "[%s] gateway_platform_event normalize error", self.name, exc_info=True
            )
            return
        if event is None:
            return
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not self.config.token:
            ...
            self._set_fatal_error(
                "missing_credentials", "No bot token configured", retryable=False
            )
            return False
        ...
        try:
            ...
            if webhook_url:
                ...
                try:
                    webhook_secret = (
                        get_secret("TELEGRAM_WEBHOOK_SECRET") or ""
                    ).strip()
                ...
                def _polling_error_callback(error: Exception) -> None:
                    ...
                    if self._looks_like_polling_conflict(error):
                        ...
                        logger.warning(
                            "[%s] Telegram network _redact_telegram_error_text(error), scheduling reconnect: %s",
                            self.name,
                            error,
                        )
                        self._polling_error_task = loop.create_task(
                        ...
                        logger.error(
                            "[%s] Telegram polling _redact_telegram_error_text(error): %s",
                            self.name,
                            error,
                            exc_info=True,
                        )
    ...
    async def _await_disconnect_step(
        self, awaitable, timeout: float, step: str
    ) -> bool:
    ...
    async def send(
        ...
        try:
                    ...
                    try:
                        ...
                        if _BadReq and isinstance(send_err, _BadReq):
                            ...
                            if (
                                ...
                                if private_dm_topic_send:
                                    safe_send_error = _redact_telegram_error_text(
                                        send_err
                                    )
                                    return SendResult(
                                        success=False,
            ...
            safe_error = _redact_telegram_error_text(e)
            logger.error(
                "[%s] Failed to send Telegram message: %s", self.name, safe_error
            )
            err_str = str(e).lower()
                    ...
                    if "reply message not found" in str(send_err).lower():
                        ...
                        try:
                            ...
                            logger.warning(
                                ...
                                self.name,
                                _redact_telegram_error_text(_retry_err),
                            )
                            sent_msg = None
                            break
                    if use_markdown:
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        if total_pages > 1:
            ...
            if page > 0:
                nav.append(
                    InlineKeyboardButton("◀ Prev", callback_data=f"mpv:{page - 1}")
                )
            nav.append(
                InlineKeyboardButton(
            ...
            if page < total_pages - 1:
                nav.append(
                    InlineKeyboardButton("Next ▶", callback_data=f"mpv:{page + 1}")
                )
            rows.append(nav)
    ...
    # get_hermes_home — multiplex profiles each get their own scripts dir).
    ...
    @staticmethod
    def _gmail_triage_script_path(script_name: str) -> _Path:
        """Profile-aware path to a gmail-triage script (F-PLUG-HOME-6).

        Must resolve via get_hermes_home() so non-default profiles look in
        their own home, not the default ~/.hermes.
        """
        try:
            from hermes_constants import get_hermes_home
        except (ModuleNotFoundError, ImportError):  # pragma: no cover
            return _Path.home() / ".hermes" / "scripts" / "gmail-triage" / script_name
                ...
                if ext in {".ogg", ".opus"}:
                    ...
                    for _cap_text, _cap_parse_mode in _caption_variants:
                        try:
                            ...
                            if _cap_parse_mode is not None and (
                                "parse" in str(_cap_error).lower()
    ...
    def _record_typing_cooldown(self, chat_id: str, exc: Exception) -> None:
        ...
        try:
            delay = (
                float(retry_after)
                if retry_after is not None
                else self._telegram_typing_cooldown_seconds
            )
        except (TypeError, ValueError):
    ...
    def _effective_message_thread_id(cls, message: Message) -> Optional[str]:
        ...
        chat = getattr(message, "chat", None)
        chat_type = (
            str(getattr(chat, "type", "")).split(".")[-1].lower() if chat else ""
        )
        raw = getattr(message, "message_thread_id", None)
        is_topic_message = bool(getattr(message, "is_topic_message", False))
        is_forum_group = (
            chat_type in ("group", "supergroup")
            and getattr(chat, "is_forum", False) is True
        )
        if raw is not None:
            if is_forum_group or (
                chat_type in ("group", "supergroup") and is_topic_message
            ):
                return str(raw)
    ...
    async def _refresh_bot_identity(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            logger.debug(
                ...
                self.name,
                self._current_bot_username() or "unknown",
                exc,
            )
            return
        self._bot_identity_checked_at = time.monotonic()
    ...
    def _extract_bot_mention_usernames(
        cls, message: Message, self_username: str = ""
    ) -> set[str]:
        ...
        for source_text, entities in _iter_sources():
            for entity in entities:
                ...
                entity_text = TelegramAdapter._telegram_entity_text(
                    source_text, offset, length
                ).strip()
        ...
        for raw_text, entities in _iter_sources():
            ...
            for match in re.finditer(
                r"(?i)(?<![A-Za-z0-9_`/])@([A-Za-z0-9_]{2,31})\b", raw_text
            ):
        ...
        for source_text, entities in _iter_sources():
            for entity in entities:
                ...
                if entity_type == "mention" and expected:
                    ...
                    if (
                        self
                        ._telegram_entity_text(source_text, offset, length)
                        .strip()
                        .lower()
                        == expected
                    ):
                    ...
                    if offset < 0 or length <= 0:
                        continue
                    command_text = self._telegram_entity_text(
                        source_text, offset, length
                    )
...
def _apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
    ...
    group_allowed_users = telegram_cfg.get("group_allow_from") or _telegram_extra.get(
        "group_allow_from"
    )
    ...
    group_allowed_chats = telegram_cfg.get(
        "group_allowed_chats"
    ) or _telegram_extra.get("group_allowed_chats")
